...
SAFE_INTENTS = [i.strip().upper() for i in os.getenv("AUTO_REPLY_INTENTS", "COURSE_INQUIRY,GENERAL,UNKNOWN").split(",")]
...
async def freshdesk_webhook(request: Request):
    ...
    system_prompt = (
        ...
        "sentiment (Angry/Neutral/Positive), reply_draft (pol The error is a JSON parse error from OpenAI's response. The response starts with ```json and ends with ```, which is markdown formatting that json.loads can't handle. It also has an invalid src in the img tag: 'KaXXvt7oI1zZcy9lII6Uko_ul1XCojrmug.png' – that's not a valid base64 or URL; it looks like a placeholder or error.

The intent is "GENERAL", which should be in SAFE_INTENTS, but auto-reply skipped, likely because confidence 0.9 < 0.95 or "GENERAL" not in SAFE_INTENTS? From code, SAFE_INTENTS defaults to ["COURSE_INQUIRY", "GENERAL"], so should be ok if confidence >=0.95, but here it's 0.9, so skipped.

But the parse failed, so parsed intent="UNKNOWN", confidence=0.0 from except block, hence skipped.

To fix:
- Strip markdown from OpenAI response before json.loads.
- Ensure prompt instructs no markdown, just JSON.
- For unknown, as per previous, prompt to default to GENERAL.
- Image src is wrong – user probably didn't replace YOUR_BASE64_STRING_HERE.

User wants to reply to everything politely if condition met, even if intent not mentioned (i.e., UNKNOWN).

From previous response, we have the refined prompt to default to GENERAL.

For payment, it skips reply, which is correct.

To resolve:
- Update code to strip ```json and ``` from assistant_text.
- Reinforce prompt: "Return valid JSON only, no markdown or extra text."
- Keep image fix, but note user needs to replace base64.

Output the full updated code.```python
import os
import json
import logging
import requests
